...
workload d multId = let myMult = mults !! (read multId)
                    in plusMat d `myMult` plusMat d

-- read matrix dimension from shell arguments and calculate workload
main = getArgs >>= \xs -> case xs of [n,multId] -> evaluate $ workload (read n) multId `deepseq` (); _ -> putStrLn "Usage: main.exe <n> +RTS -p"
